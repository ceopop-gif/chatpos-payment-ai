...
async function seedInitialTables(merchantId: string) {
  const db = getD1();
  const count = await db.prepare("SELECT COUNT(*) AS total FROM restaurant_tables WHERE merchant_id = ?").bind(merchantId).first<{ total: number }>();
  if (Number(count?.total ?? 0) > 0) return;
  await db.batch(Array.from({ length: 8 }, (_, index) => db.prepare(
    "INSERT INTO restaurant_tables (merchant_id, name, token, active) VALUES (?, ?, ?, 1)"
  ).bind(
    merchantId,
    `โต๊ะ ${index + 1}`,
...
export async function GET(request: Request) {
  try {
    ...
    await seedInitialTables(session.applicationId);
    const result = await getD1().prepare(`
      ...
      WHERE t.active = 1 AND t.merchant_id = ?
      GROUP BY t.id
      ORDER BY
...
export async function POST(request: Request) {
  try {
    ...
    const db = getD1();
    const duplicate = await db.prepare("SELECT id FROM restaurant_tables WHERE merchant_id = ? AND active = 1 AND name = ? LIMIT 1").bind(session.applicationId, name).first();
    if (duplicate) return Response.json({ error: "มีชื่อโต๊ะนี้แล้ว" }, { status: 409 });

    const token = crypto.randomUUID().replace(/-/g, "") + crypto.randomUUID().replace(/-/g, "");
    const table = await db.prepare(
      "INSERT INTO restaurant_tables (merchant_id, name, token, active) VALUES (?, ?, ?, 1) RETURNING id, name, token, active, created_at"
    ).bind(session.applicationId, name, token).first();
...
export async function PATCH(request: Request) {
  try {
    ...
    const db = getD1();
    const current = await db.prepare("SELECT id FROM restaurant_tables WHERE id = ? AND merchant_id = ? AND active = 1 LIMIT 1").bind(id, session.applicationId).first();
    if (!current) return Response.json({ error: "ไม่พบโต๊ะนี้ในระบบ" }, { status: 404 });

    const duplicate = await db.prepare("SELECT id FROM restaurant_tables WHERE merchant_id = ? AND active = 1 AND name = ? AND id <> ? LIMIT 1").bind(session.applicationId, name, id).first();
    if (duplicate) return Response.json({ error: "มีชื่อโต๊ะนี้แล้ว" }, { status: 409 });

    const table = await db.prepare(
      "UPDATE restaurant_tables SET name = ? WHERE id = ? AND merchant_id = ? RETURNING id, name, token, active, created_at"
    ).bind(name, id, session.applicationId).first();
